...
export const PHASES = [
  ...
  },
  {
    ...
    points: [
      ...
      { label: "Keys that repeat", text: "A column whose name contains the word id, uuid, key or ref — split on camelCase and punctuation, so MeetID counts — or is named just index, but whose values repeat is a key into another table or an entity: above 50 distinct values it is an identifier (openpowerlifting's MeetID, 8,482 meets), at or below it a categorical group such as a store_id, and never numeric, because an id's arithmetic means nothing. 50 is a stated default, not a measured threshold." },
  ...
  },
  {
    ...
    points: [
      ...
      { label: "Leakage", text: "A feature is a leakage suspect at |r| ≥ 0.95 or Cramér's V ≥ 0.95 against the target — after the corrections above, both mean near-determinism. An r flag must also hold on ranks (|ρ| ≥ 0.9), because a single extreme row can carry r to 1 on its own. A presence indicator at V ≥ 0.95 is flagged the same way. η is deliberately excluded: η near 1 also occurs for genuinely strong categorical predictors." },
  ...
  },
  {
    ...
    points: [
      ...
      { label: "Evidence caps", text: "Under 10 rows holds the score to 25, under 50 to 50, under 200 to 75. A worst column that is 50% or more empty imposes a ceiling that slides with severity — 89 at 50%, 60 at 90%, 45 at 100% — so datasets with unlike problems do not collapse onto one number. A target that never varies holds it to 25, and one that is unique per row to 40." },
      { label: "Readiness caps", text: "A suspected leak — from the correlation scan, a presence signal, or a column the baseline model predicts the target from almost perfectly on its own — holds the score to 70. A baseline that does not beat a know-nothing guess holds it to 74. Both sit below Good: a file that may carry its own label, or whose columns show no signal, is not ready to train on however clean it looks." },
  ...
  },
  {
    ...
    points: [
      ...
      { label: "Is there signal", text: "A paired t-test across the folds with the Nadeau–Bengio correction for folds that share training rows; signal needs t ≥ 2.78 (p < 0.05). No signal means the easy, linear signal is absent — not that none exists. For a numeric target, if predicting the training average scores below −1 in R² in any fold — it scores about 0 on an ordinary target — a few extreme values put the folds on different scales, and no score or verdict is given." },
      ...
      { label: "Same target within a repeated value", text: "For every column with more than 50 distinct values, the rows that share a value: when at least 20 rows repeat and the target is the same within them far beyond its usual rate — (same − most common) / (1 − most common) ≥ 0.9, counted over the rows where the column is filled — the column is reported to the leakage review and to you. It is evidence, not a verdict: an entity a random split would memorise, a value only known after the outcome, or harmless. Over the 40 test files with a target it named three columns, each a documented case." },
    ],
  },
];
