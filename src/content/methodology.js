...
export const PHASES = [
  ...
  },
  {
    ...
    points: [
      { label: "Metric by type", text: "Types are the column roles assigned earlier — the scan never re-guesses them. Numeric or binary feature against a numeric or binary target: Pearson (point-biserial when one side is 0/1) with Spearman beside it. Numeric against categorical, in either direction: the correlation ratio η with the degrees-of-freedom (epsilon-squared) correction, so a feature with thousands of levels cannot climb on cardinality, and a p-value from its one-way ANOVA F-test. Categorical against a categorical or binary target: bias-corrected Cramér's V. A date or free-text target is not scored. Binary levels are sorted before 0/1 encoding, so the sign of r never depends on row order." },
  ...
  },
  {
    ...
    points: [
      ...
      { label: "One decision per column", text: "A column that is dropped (identifier, constant, empty), replaced by a presence indicator, or under leakage investigation gets no further advice — no outlier, skew or imputation step for a column already on its way out, and no pair advice that assumes it stays. Leakage outranks everything else about a column." },
      { label: "Direction", text: "Cramér's V is symmetric but dependency is not. When one column has at least three times the levels of the other, a high V means the finer column determines the coarser one, and the advice says so instead of \"keep one of the two\". Columns of similar grain keep that advice." },
